...
$.ajax({
	type: "post",
	url: "http://testapi.yiuxiu.com/Game/GetGameAdvs",
	async: false,
	success:function(str){
		data = str.data;
		curImg = data[0].ImageUrl;
...
$.ajax({
	type:"post",
	url:"http://testapi.yiuxiu.com/User/GetCoins",
	async:false,
	data:{
		UserID:userId,
		Sign:''
...
puzzleGame.prototype = {
	...
	levelSelect: function() {
		...
		var self = this;
		this.btnLevel.change(function() {
			if(self.hasStart) {
				return false;
			}

			self.levelNow = this.selectedIndex;
			var sen = self.countdown[this.selectedIndex];
	...
	gameState: function() {
		...
		this.btnStart.on('click', function() {
			console.log(accum,curImg)
			//判断用户优点数，小于20不能玩，目前取消，等以后需要再开。
//			if(accum<20){return;}
			//判断用户是否登录
			if(unLogin == false){
				$('#unLogin').show();
			...
			if(self.hasStart ==1){return}
			$.ajax({
				type:"post",
				url:"http://testapi.yiuxiu.com/Game/GetAPICallCount",
				data:{
					UserID:userId
				},
				success:function(data){
...
//						$.ajax({
//						type:"post",
//						url:"http://testapi.yiuxiu.com/Game/SubtractYouDian",
//						data:{
//							UserID:userId,
//							Num:20,
//							GameName:'拼图'
...
//					}
	...
	success: function() {
		...
		$.ajax({
			type:"post",
			url:"http://testapi.yiuxiu.com/Game/RewardYouDian",
			data:{
				UserId:userId,
			...
			success:function(){
				accum += parseFloat($("select").val());
				$('.special').show();
				$('.special img').attr('src',curImg)
				timer1 = setInterval(function(){
					$('.special img').css({'box-shadow':'0px 0px 50px #f00'})
				},1000)
				timer2 = setInterval(function(){
					$('.special img').css({'box-shadow':'0px 0px 0px #f00'})
				},1300)
				timer3 = setInterval(function(){
					$('.special img').css({'box-shadow':'0px 0px 50px #0000EE'})
				},1600)
